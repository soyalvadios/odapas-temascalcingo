...
const WHATSAPP_NUMBER = "5212207402561";

type Address = {
  cp: string; estado: string; municipio: string;
  colonia: string; calle: string; no_ext: string;
  no_int: string; referencia: string;
};

const ADDR_DEFAULT: Address = {
  cp: "", estado: "Estado de México", municipio: "Temascalcingo",
  colonia: "", calle: "", no_ext: "", no_int: "", referencia: "",
};
...
function buildWhatsAppMessage(p: {
  ...
  correo: string; telefono: string; tipo: string;
  descripcion: string; addr: Address; geo: LatLng;
}): string {
  const nombre = [p.nombre, p.apPaterno, p.apMaterno].filter(Boolean).join(" ");
  const dir = [
    p.addr.calle && `${p.addr.calle}${p.addr.no_ext ? " #" + p.addr.no_ext : ""}${p.addr.no_int ? " Int. " + p.addr.no_int : ""}`,
    p.addr.colonia && `Col. ${p.addr.colonia}`,
    p.addr.cp && `C.P. ${p.addr.cp}`,
    p.addr.municipio, p.addr.estado,
  ].filter(Boolean).join(", ");
  const maps = `https://maps.google.com/?q=${p.geo.lat.toFixed(6)},${p.geo.lng.toFixed(6)}`;
  return [
    ...
    p.addr.referencia ? `• Referencia: ${p.addr.referencia}` : null,
    `• Mapa: ${maps}`,
...
export default function Reportar() {
  ...
  async function useMyLocationAndFill() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setAutoFilling(false);
        if (err.code === 1)
          showStatus("Permiso denegado. Ve a Configuración de tu navegador y permite la ubicación para este sitio.", "error");
        else if (err.code === 2)
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setCorreo(""); setTelefono(""); setTipo("Fuga de agua");
    setDescripcion(""); setAddr(ADDR_DEFAULT); setGeo(defaultCenter); setAgreed(false);
    ...
    <div className="container page">
      ...
      <form onSubmit={onSubmit} className="reportGrid">
        ...
        <div className="card pad">
          ...
          {/* BOTÓN UBICACIÓN — al inicio del form, visible sin scroll en móvil */}
          <button
          ...
          <div className="row2">
            <div>
              ...
              <input className="input" name="correo" type="email" placeholder="correo@ejemplo.com"
          ...
          <textarea className="input" name="descripcion" rows={4} required
          ...
          <div className="row3">
            ...
            <div>
              ...
              <input className="input" name="referencia" value={addr.referencia}
                onChange={(e) => setField("referencia", e.target.value)} placeholder="Entre calles, frente a..." />
            </div>
          </div>
        ...
        <div className="card pad">
          ...
          {/* Sin key dinámica — el mapa nunca se destruye al cambiar geo */}
